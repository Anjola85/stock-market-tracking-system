...
function validatePhone(phone_number){
    if (phone_number.length == 10 || phone_number.length <= 12){
        ...
        if(check) {//add country code +1
            phone_number = `+1${phone_number}`;
        } 
        return phone_number;
    } else {
        console.log("PHONE NUMBER IS NOT REAL!!! => FROM validatePhone Function in validator/user.js");
        return null;
    }
}
...
function update (body) {//to update user information
    const rules = {};
    const validator = new Validator(body, rules);
    return validator;
}
